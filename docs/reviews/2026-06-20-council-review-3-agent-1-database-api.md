...
The repository contains **79 database migrations** under `supabase/migrations/`.
...
- **Recently Added Tables (HQ Governance)**:
  - `hq_sessions`
  - `hq_tasks`
  - `hq_risks`
  - `hq_decisions`
...
- **`lib/actions/audit.ts`**: Implements `logAuditEvent`. Now supports `"READ_PASTORAL"` operation.
...
1. **Audit Log Pruning Policy**: With the introduction of read-auditing, the `audit_log` table will experience rapid growth. We need a retention policy (e.g. 1 year) and an automatic pruning schedule.
2. **Bulk Upload Rate Limiting**: The CSV import routes for member/financial data should have stricter transaction size limits and rate limiting to prevent memory depletion.
3. **Database Health Metrics API**: We need a read-only endpoint monitoring database connection pools and query queue depths.
4. **Session Expiry Synchronization**: Client-side timeout (15 mins) must be synchronized with Supabase JWT expiry parameters.
5. **Read Audit Log Coverage**: Add read-auditing to other highly sensitive sections, such as child check-in security logs.
